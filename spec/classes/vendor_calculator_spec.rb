...
RSpec.describe VendorCalculator, type: :class do
  ...
  context 'vendor with currency of UAH' do
    ...
    it "total_money" do
      expect(vendor_calculator.total_money).to eq('7789.92')
    end

    it "total_money_words" do
      expect(vendor_calculator.total_money_words('7789.92')).to eq('Сiм тисяч сiмсот вiсiмдесят дев’ять гривень')
    end

    it "params_new_act" do
      expect(vendor_calculator.params_new_act).to eq({ vendor_id: vendor.id , total_money: "7789.92", month: '6/2015'.to_date })
    end
  end
end
